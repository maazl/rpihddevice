...
#define CLOG(notice, comp, port, msg, ...) do { \
	...
	else fprintf(stderr, "[%lx:%p] %s: " msg "\n", syscall(SYS_gettid), comp, __func__ , ##__VA_ARGS__); \
} while (0)
#endif
...
static OMX_ERRORTYPE gomx_get_parameter(OMX_HANDLETYPE hComponent, OMX_INDEXTYPE nParamIndex, OMX_PTR pComponentParameterStructure)
{
	...
	{	OMX_AUDIO_PARAM_PORTFORMATTYPE *pft;
		if ((r = omx_cast(pft, pComponentParameterStructure))) return r;
		if (!(port = gomx_get_port(comp, pft->nPortIndex))) return OMX_ErrorBadPortIndex;
		pft->nIndex = 0;
		pft->eEncoding = OMX_AUDIO_CodingPCM;
		break;
	}
	default:
		...
		OMX_HANDLETYPE hTunneledComp, OMX_U32 nTunneledPort, OMX_TUNNELSETUPTYPE* pTunnelSetup)
{
	...
	CDEBUG(comp, 0, "called , %u, %p, %u, {%x, %i}", nPort, hTunneledComp, nTunneledPort, pTunnelSetup ? pTunnelSetup->nTunnelFlags : 0, pTunnelSetup ? pTunnelSetup->eSupplier : 0);
				     ...
				     OMX_U32 nPortIndex, OMX_PTR pAppPrivate, OMX_U32 nSizeBytes, OMX_U8* pBuffer)
{
	...
	if (!( comp->state == OMX_StateIdle ||
		...
		(port->def.bEnabled == OMX_FALSE && (comp->state == OMX_StateExecuting || comp->state == OMX_StatePause)) ))
		return OMX_ErrorIncorrectStateOperation;
...
static OMX_ERRORTYPE __gomx_empty_buffer_done(GOMX_COMPONENT *comp, OMX_BUFFERHEADERTYPE *hdr)
{
	...
	if (port->tunnel_comp) {
		...
		pthread_mutex_unlock(&comp->mutex);
		r = comp->cb.EmptyBufferDone((OMX_HANDLETYPE) comp, comp->omx.pApplicationPrivate, hdr);
		pthread_mutex_lock(&comp->mutex);
	}
...
{
	CDEBUG(hComponent, 0, "gomx_fill_this_buffer");
	return OMX_ErrorNotImplemented;
}
...
static OMX_ERRORTYPE __gomx_port_populate(GOMX_COMPONENT *comp, GOMX_PORT *port)
{
	...
	if (port->tunnel_supplier) {
		CINFO(comp, port, "Allocating tunnel buffers: %zu", port->num_buffers);
...
static OMX_ERRORTYPE gomx_send_command(OMX_HANDLETYPE hComponent, OMX_COMMANDTYPE Cmd, OMX_U32 nParam1, OMX_PTR pCmdData)
{
	...
	// Worker causes deadlock when the PortDisable command comes before the first buffer is ready.
	// Work around: signal the buffer ready cond var.
	GOMX_PORT* port;
	if (Cmd == OMX_CommandPortDisable && (port = gomx_get_port(comp, nParam1)))
	{	port->def.bPopulated = OMX_TRUE;
		pthread_cond_signal(&port->cond_populated);
	}
	pthread_mutex_unlock(&comp->mutex);
	CINFO(comp, 0, "SendCommand complete");
	...
	while (comp->state != OMX_StateInvalid) {
		...
		if (cmd) {
			...
			if (r == OMX_ErrorNone)
				...
				__gomx_event(comp, OMX_EventError, r, 1, 0);
		} else {
...
{
	GOMX_COMPONENT *comp = (GOMX_COMPONENT *) hComponent;
	CINFO(comp, 0, "gomx_set_callbacks(%p, ,%p)", hComponent, pAppData);
	if (comp->state == OMX_StateInvalid) return OMX_ErrorInvalidState;
...
{
	CINFO(comp, 0, "gomx_init(%p, %p, , , %d)", comp, pAppData, nports);
...
static OMX_ERRORTYPE omxalsasink_set_parameter(OMX_HANDLETYPE hComponent, OMX_INDEXTYPE nParamIndex, OMX_PTR pComponentParameterStructure)
{
	...
	{	OMX_PARAM_PORTDEFINITIONTYPE *pdt;
		...
		port->def.nBufferCountActual = pdt->nBufferCountActual;
		break;
	}
	case OMX_IndexParamAudioPortFormat:
	{	OMX_AUDIO_PARAM_PORTFORMATTYPE *pft;
		if ((r = omx_cast(pft, pComponentParameterStructure))) return r;
		if (!(port = gomx_get_port(comp, pft->nPortIndex))) return OMX_ErrorBadPortIndex;
		if (pft->nPortIndex != OMXALSA_PORT_AUDIO) return OMX_ErrorBadParameter;

		if (comp->state != OMX_StateLoaded && port->def.bEnabled)
			return OMX_ErrorIncorrectStateOperation;

		if (pft->eEncoding != OMX_AUDIO_CodingPCM) // only PCM is supported
	...
	{	OMX_AUDIO_PARAM_PCMMODETYPE *pmt;
		...
		CDEBUG(comp, 0, "OMX_IndexParamAudioPcm %u, %x, %x, %u, %u, %u -> %i",
			pmt->nChannels, pmt->eNumData, pmt->eEndian, pmt->bInterleaved, pmt->nBitPerSample, pmt->nSamplingRate, pcm_format);
